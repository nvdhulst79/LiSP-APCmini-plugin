...
- **Per-cue running color override** — same UI as the idle-color picker but for the running state. Useful for color-coding by track (FX blue, music green, etc.). Marginal: your eye already finds the playing pad because it's bright. Cost: low — one more combo in the cue tab.
- **Per-cue "disable binding"** — checkbox to opt a cue out of pad mapping entirely. Niche; skip until someone asks. Cost: low.
- **Out of v1 scope (Phase 2):** fader mappings (8 faders for cue/master volume), Scene Launch / Track Buttons, Shift modal combos (e.g., Shift+pad to "move cart from-to"), APC mini mk1 support (different palette + monochrome LEDs), per-cue color from the cue's `Appearance` tab (auto-derive from LiSP's existing color picker rather than maintaining a separate one).
